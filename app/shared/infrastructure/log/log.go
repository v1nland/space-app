...
func WithErrors(errs []error) *Logger { return instance.WithErrors(errs) }
func (logger *Logger) WithErrors(errs []error) *Logger {
	logCustom := &Logger{
		log: initLoggerZap(),
	}
	logCustom.log = logCustom.log.With(zap.Any("errors", errs))
	return logCustom
}
